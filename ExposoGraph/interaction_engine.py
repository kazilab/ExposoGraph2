...
def enzyme_induction_modifier(
    ...
    return EnzymeInductionProfile(
        enzyme_folds={
            key: np.round(value, 3) for key, value in sorted(enzyme_folds.items())
        },
    ...
    *,
    ...
    for sub_name, conc_uM in substrates.items():
        ...
        results[sub_name] = SubstrateFluxChange(
            single_flux=np.round(single_flux, 6),
            competitive_flux=np.round(competitive_flux, 6),
            flux_change_fraction=np.round(flux_change_fraction, 4),
            inhibition_term=np.round(inhibition_term, 4),
            activated_product_flux=np.round(
    ...
    *,
    ...
    for rate_key, flux_umol_h_g in rate_map.items():
        ...
        contributions[consumer_key] = {
            "substrate_flux_umol_h_g": np.round(substrate_flux, 4),
            "gsh_consumption_umol_h_g": np.round(gsh_drain, 4),
    ...
    for contribution in contributions.values():
        if total_consumption > 0:
            contribution["fraction_of_total"] = np.round(
    ...
    if total_consumption > synthesis_rate:
        ...
        time_to_depletion_h = np.round(gsh_to_lose / net_drain, 2)
    ...
    return GSHStatus(
        ...
        steady_state_gsh_mM=np.round(steady_state_mM, 3),
        fraction_normal=np.round(fraction_normal, 3),
        consumption_rate_umol_h_g=np.round(total_consumption, 4),
        synthesis_rate_umol_h_g=synthesis_rate,
        net_rate_umol_h_g=np.round(net_rate, 4),
        consumption_exceeds_synthesis=tipping_point_reached,
        tipping_point_reached=tipping_point_reached,
        tipping_point_multiplier=(
            np.round(total_consumption / synthesis_rate, 3)
            if synthesis_rate > 0
            else 0.0
        ),
...
def gsh_depletion_biology_model(
    ...
    *,
    ...
    tipping_point_multiplier = (
        np.round(total_consumption / v_max_synth, 3) if v_max_synth > 0 else 0.0
    )
        ...
        while t < max_t:
            ...
            if G_next <= G_critical:
                if G > G_next:
                    ...
                    time_to_depletion_h = np.round(t + frac * dt, 2)
                else:
                    time_to_depletion_h = np.round(t, 2)
                break
            if abs(dG) < 1e-9:
                break
            G = G_next
    ...
    return GSHStatus(
        ...
        fraction_normal=np.round(fraction_normal, 3),
        consumption_rate_umol_h_g=np.round(total_consumption, 4),
        synthesis_rate_umol_h_g=np.round(synth_at_ss, 4),
        net_rate_umol_h_g=np.round(net_rate, 4),
...
def _compute_gsh_detox_penalty(
    ...
    return np.round(genotype_factor * gsh_penalty, 3)
...
def compute_interaction_matrix(
    ...
    *,
    ...
    for carcinogen, value in normalized_exposure.items():
        ...
        individual_risks[carcinogen] = np.round(
    ...
    for carcinogen in present_carcinogens:
        ...
        interaction_adjusted_risks[carcinogen] = np.round(
    ...
    for index, left in enumerate(present_carcinogens):
        for right in present_carcinogens[index + 1 :]:
            ...
            synergy_matrix[f"{left}_x_{right}"] = np.round(
                adjusted_total / independent_total if independent_total > 0 else 1.0,
                3,
            )

    total_independent = np.round(sum(individual_risks.values()), 3)
    total_interaction = np.round(sum(interaction_adjusted_risks.values()), 3)
    interaction_factor = np.round(
    ...
    *,
    ...
    for pair, composite in full_result.synergy_matrix.items():
        ...
        decomposed[pair] = SynergyDecomposition(
            ...
            delta_comp=np.round(delta_comp, 4),
            delta_gsh=np.round(delta_gsh, 4),
            delta_ind=np.round(delta_ind, 4),
            additive_estimate=np.round(additive, 4),
            residual=np.round(composite - additive, 4),
        )
    return decomposed
    ...
    *,
    ...
    def _summary(values: list[float]) -> tuple[float, tuple[float, float]]:
        ...
        return np.round(mean, 4), (
            np.round(np.percentile(values, 2.5), 4),
            np.round(np.percentile(values, 97.5), 4),
        )
